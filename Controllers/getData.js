...
try{
const target=await company.findOne({CompanyName:companyName});
res.json(target);
}catch(err){
...
try{
const reports=await fraudReport.find().sort({reportDate:-1}).limit(1);
const reportsWithStatus=await Promise.all(reports.map(async(report)=>{
const int=await internship.findOne({internshipTitle:report.internshipTitle,companyName:report.companyName});
return{...report.toObject(),internshipStatus:int?.status||'Unknown'};
}));
res.json({success:true,reports:reportsWithStatus});
}catch(err){
...
const {companyName,internshipTitle,reportedBy,issue,severity}=req.body;
try{
const newReport=new fraudReport({companyName,internshipTitle,reportedBy,issue,severity});
...
try{
const reports=await fraudReport.find({status:{$ne:'Resolved'}}).sort({reportDate:-1});
const reportsWithStatus=await Promise.all(reports.map(async(report)=>{
const int=await internship.findOne({internshipTitle:report.internshipTitle,companyName:report.companyName});
return{...report.toObject(),internshipStatus:int?.status||'Unknown'};
}));
...
const companyNames=await company.find({Location:new RegExp(region,'i')}).select('CompanyName');
const regionFraudCount=await fraudReport.countDocuments({companyName:{$in:companyNames.map(c=>c.CompanyName)}});
return{region,companyCount,fraudCount:regionFraudCount};
}));
...
if(status==='Inactive'){
await company.findOneAndUpdate({CompanyName:companyName},{$inc:{trustScore:-3}});
}
...
const {companyName,action:actionType,reason,status}=req.body;
try{
const newAction=new action({companyName,action:actionType,reason,status,actionDate:new Date()});
...
const updateFraudReportStatus=async(req,res)=>{
const {companyName,internshipTitle,status}=req.body;
try{
await fraudReport.findOneAndUpdate({companyName,internshipTitle},{status});
res.json({success:true,message:'Fraud report status updated'});
}catch(err){
console.error("Error updating fraud report:",err);
res.status(500).json({success:false,error:err.message});
}
}
module.exports={
